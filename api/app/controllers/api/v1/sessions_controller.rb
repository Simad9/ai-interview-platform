...
module Api
  module V1
    class SessionsController < ApiController
      ...
      # DELETE /api/v1/assessments/:assessment_id/sessions/:id
      def destroy
        assessment_id = params[:assessment_id].to_i
        return json_error('Session not found', :not_found) if assessment_id.positive? &&
                                                              assessment_id != @session.assessment_id

        return json_error('Session is currently in progress', :unprocessable_entity) if @session.active?
